...
public class Chunk {
	...
	private boolean surround(int x, int y, int z){
		...
		temp = (y>0) ? cubes[x][y-1][z]!=null && temp : false;//clone.getChunkManager().getCubeAt(cubes[x][y][z].getX(), cubes[x][y][z].getY()+1, cubes[x][y][z].getZ())==null && temp || (clone.getChunkManager().chunkExist(this.x, this.y+1, this.z));

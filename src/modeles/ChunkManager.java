...
public class ChunkManager implements Parametres {
	...
	/**
	 * Trouve le plus "haut" chunk, génère les cubes puis luis demande sont plus
	 * haut point.
	 * 
	 * @param x
	 *            Coordonée X de la caméra
	 * @param z
	 *            Coordonée Z de la caméra
	...
	private boolean chunksurround(Chunk ck) {
		...
		int zCh = ck.getZ();
		boolean temp = true;
		temp = (chunkExist(xCh + 1, yCh, zCh)) ? temp
				&& completeFace(xCh + 1, yCh, zCh, 5) : false;
		temp = (chunkExist(xCh - 1, yCh, zCh)) ? temp
				&& completeFace(xCh - 1, yCh, zCh, 6) : false;
		temp = (chunkExist(xCh, yCh + 1, zCh)) ? temp
				&& completeFace(xCh, yCh + 1, zCh, 4) || yCh + 1 > 0 : false;
		temp = (chunkExist(xCh, yCh - 1, zCh)) ? temp
				&& completeFace(xCh, yCh - 1, zCh, 3) : false;
		temp = (chunkExist(xCh, yCh, zCh + 1)) ? temp
				&& completeFace(xCh, yCh, zCh + 1, 1) : false;
		temp = (chunkExist(xCh, yCh, zCh - 1)) ? temp
				&& completeFace(xCh, yCh, zCh - 1, 2) : false;
		return temp;
	}
	...
	/**
	 * Fonction initiale de récursive. Vérifie tout les chunk entre le joueur et
	 * le chunk à vérifier.
	 * 
	 * @param xMove
	 *            Facteur de déplacement X
	 * @param zMove
	...
	public void createChunks(int xMove, int zMove) {
		...
		if (Math.abs(xMove) < Math.abs(zMove)) {
			for (int i = 1; i <= Math.abs(zMove); i++) {
				if (chunkExist((int) pos.x + xMove, (int) pos.y + 1,
					...
					// (int)pos.y+1, (int)pos.z+(int)(Math.signum(zMove)*i));

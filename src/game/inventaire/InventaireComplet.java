...
public class InventaireComplet implements Inventaire {
	...
	//les tableaux de collection de generique, Java il ne kiff vraiment pas �a
	...
	//plus facile a utilis� mais quelque probleme d'un POV logique ( le sac d'armes est un sac d'item :{ bofbof)
	...
	public boolean addItem(Item item, Inventaire inventaire) {
		...
		if(inventaire.contains(item)){
			...
			done = this.inventaire.get(item.getIdentifiant()).add(item);
		}
		return done;
	}
	...
	public boolean addItem(Item item, int position, Inventaire inventaire) {
		Item temp;
		try {
			temp = this.inventaire.get(item.getIdentifiant()).getSac().get(position);
